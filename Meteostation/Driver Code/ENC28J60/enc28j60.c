...
#define enc28j60_select() GPIO_ResetBits(ENC28J60_SPI_CS_PORT, ENC28J60_SPI_CS_PIN)
#define enc28j60_release() GPIO_SetBits(ENC28J60_SPI_CS_PORT, ENC28J60_SPI_CS_PIN)

uint8_t enc28j60_rxtx(uint8_t data)
{
	while(SPI_I2S_GetFlagStatus(ENC28J60_SPI_PORT, SPI_I2S_FLAG_TXE)==RESET);
	SPI_I2S_SendData(ENC28J60_SPI_PORT,data);

	while(SPI_I2S_GetFlagStatus(ENC28J60_SPI_PORT, SPI_I2S_FLAG_RXNE)==RESET);
	return SPI_I2S_ReceiveData(ENC28J60_SPI_PORT);
}
...
void enc28j60_init(uint8_t *macadr)
{
	...
	// Initialize PORT SPI 
	RCC_APB2PeriphClockCmd(ENC28J60_SPI_PORT_CLOCK | RCC_APB2Periph_AFIO, ENABLE);
	RCC_APB2PeriphClockCmd(ENC28J60_RESET_CLOCK, ENABLE);
        RCC_APB2PeriphClockCmd(ENC28J60_SPI_CS_CLOCK, ENABLE);
        
        
#if _SPI_ENC28J60_PORT == 2   ||    _SPI_ENC28J60_PORT == 3     //RCC_APB1Periph_SPI2 or RCC_APB1Periph_SPI3
      RCC_APB1PeriphClockCmd(SPI_CLOCK, ENABLE);   
#endif
#if _SPI_ENC28J60_PORT == 1     //RCC_APB2Periph_SPI1
	...
	// CS
	GPIO_InitStruct.GPIO_Pin = ENC28J60_SPI_CS_PIN;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;

	GPIO_Init(ENC28J60_SPI_CS_PORT, &GPIO_InitStruct);

	// RESET
	GPIO_InitStruct.GPIO_Pin = ENC28J60_RESET_PIN;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(ENC28J60_RESET_PORT, &GPIO_InitStruct);
	...
	SPI_Init(ENC28J60_SPI_PORT, &SPI_InitStruct);


	SPI_Cmd(ENC28J60_SPI_PORT, ENABLE);
